...
2. **读取 `01-大纲.md`** — 找到当前章节的规划信息，提取：核心事件、承接上章、悬念钩子、出场人物、场景列表。**重点检查时间线、伏笔追踪表和人物进阶条件**
2.5. **读取 `00-人物档案.md`** — 根据大纲中本章的「出场人物」列表，提取每个出场角色的：性格核心、致命缺陷、说话风格/口头禅、恐惧/弱项、与其他角色的关系
3. **强制执行章节衔接检查**：读取上一章文件，100%精准承接上章结尾情绪、对话状态、场景氛围、遗留伏笔，记录上章结束时的人物位置和情绪状态，确保本章开篇无缝衔接
4. 更新 `02-写作计划.json` — 将本章 `status` 设为 `"in_progress"`

### 步骤 2: 撰写

5. 创建章节文件 — 文件名格式：`第{XX}章-{章节标题}.md`（标题来自 `02-写作计划.json` 中的 `title` 字段），使用 [chapter-template.md](../guides/chapter-template.md) 模板
...
14. 生成章节摘要 — 在 `01-大纲.md` 的章节摘要区追加（300-500字，保证连贯性参考）
15. 更新伏笔追踪表 — 标记新埋设的伏笔、已回收的伏笔
16. 更新时间线 — 确认本章日期/时间/季节与前后一致
17. 检查人物进阶条件 — 是否满足某个人物的成长触发条件？
...
2. **执行章节衔接检查**：如果前章已存在（非本章为第1章），读取上一章文件，确保开篇柔化过渡、承接上章结尾情绪
3. 读取 02-写作计划.json，确认章节状态
4. 将当前章节 status 更新为 "in_progress"
5. 创建章节文件，文件名格式：`第{XX}章-{章节标题}.md`（标题来自 `02-写作计划.json` 的 `title` 字段），基于大纲规划撰写正文（3000-5000字）
...
15. 再次字数检查
16. 在 01-大纲.md 追加 300-500 字章节摘要
...
4. **执行章节衔接检查**：如果前章已存在，读取上一章文件，确保承接上章结尾情绪与钩子
5. 读取 02-写作计划.json，获取该章节状态
6. 执行逐章创作流程:
